...
const COUNTDOWN_TARGET_MS = new Date("2026-02-19T00:00:00-06:00").getTime();
...
function getCountdown(nowMs: number): CountdownValue {
  const distance = COUNTDOWN_TARGET_MS - nowMs;

  if (distance < 0) {
    return { days: "O", hours: "V", minutes: "E", seconds: "R" };
  }
...
type StopTheStigmaContentProps = {
  freezeCountdown: boolean;
  initialNowMs: number;
};

export default function StopTheStigmaContent({
  freezeCountdown,
  initialNowMs,
}: StopTheStigmaContentProps) {
  const [countdown, setCountdown] = useState<CountdownValue>(() =>
    getCountdown(initialNowMs),
  );
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => setCountdown(getCountdown(Date.now()));
    updateCountdown();
